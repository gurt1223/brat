...
void *subrateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    FCS_flight_Fixed_Wing_step(subRateId);
...
void *baseRateTask(void *arg)
{
  runModel = (FCS_flight_Fixed_Wing_M->getErrorStatus() == (NULL));
  while (runModel) {
    ...
    if (FCS_flight_Fixed_Wing_M->StepTask(1)
        ) {
      px4_sem_post(&subrateTaskSem[0]);
    }

    FCS_flight_Fixed_Wing_step(0);

    // Get model outputs here
    stopRequested = !((FCS_flight_Fixed_Wing_M->getErrorStatus() == (NULL)));
  }
...
{
  UNUSED(sig);
  FCS_flight_Fixed_Wing_M->setErrorStatus("stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  // Terminate model
  FCS_flight_Fixed_Wing_terminate();
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  FCS_flight_Fixed_Wing_M->setErrorStatus(0);

  // Initialize model
  FCS_flight_Fixed_Wing_initialize();
