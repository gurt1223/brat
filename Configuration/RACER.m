...
vehicle.tau = 1/(2*pi*2);

load('MS_PTI.mat')

load('lqr_k_modified.mat')
vehicle.lqr_k = K;

%% L1AC Paramters
L1.samp_time = 1/200;
kad = 1;
L1.As = 1*[-5*eye(3),zeros(3,3);zeros(3,3),[-10 -10 -10].*eye(3)];
phi = inv(L1.As) * (expm(L1.As*L1.samp_time*kad) - eye(6));
mu = expm(L1.As*L1.samp_time*kad);

L1.adaptation_gain = inv(phi) * mu;

% Mixing matrix to mix F and moment commands to control inputs 
Mix.MF2U = [-0.6681, 0.6681, -1.0000, 0.2500;
0.6681, -0.6681, -1.0000, 0.2500;
0.6681, 0.6681, 1.0000, 0.2500;
-0.6681, -0.6681, 1.0000, 0.2500];
Mix.U2MF = pinv(Mix.MF2U);
